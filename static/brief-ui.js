...
export function createBriefWorkflow({state,app,api,esc,icon,safeUrl,render,run,toast,getTask,upsertTask,refresh,go,route,stored,remember,sectionHeading,ownTask,missingPage,deniedPage}){
 ...
 function questionsPanel(task){
  ...
  return `<section class="panel brief-panel" id="brief-questions"><div class="brief-panel-heading"><div><p class="eyebrow">АГЕНТ 02 · УТОЧНЕНИЯ</p><h2>${q.questions.length} ${q.questions.length===5?'вопросов':'вопроса'} для вашей задачи</h2><p class="subtle">Нужны для подбора направлений, специалистов и навыков.</p></div><span class="pill soft">${modeLabel(q.mode)}</span></div>${notice(q.warning)}<form id="qualification-form" data-task-id="${task.id}" data-set-id="${q.id}" data-revision="${task.revision}">${q.questions.map((question,i)=>`<div class="brief-question"><label for="qual-${question.id}"><span>${i+1}</span>${esc(question.question)}</label><p class="field-help"><strong>Зачем:</strong> ${esc(question.why)}</p><textarea id="qual-${question.id}" name="${question.id}" required rows="3" maxlength="3000" placeholder="Ваш ответ. Если пока неизвестно, так и напишите.">${esc(answers[question.id]||'')}</textarea><details><summary>Основание и проверка вопроса</summary>${question.evidence.map(e=>`<blockquote>${esc(e.quote)}</blockquote>`).join('')}<p class="field-help">Проверены: связь с задачей, отсутствие ответа в тексте, обоснованность, безопасность, цель и нейтральность. ${question.checkedBy==='rules'?'Проверка по правилам.':'Проверка моделью и правилами.'}</p></details></div>`).join('')}<div class="brief-actions"><span>Можно ответить «пока неизвестно»</span><button class="btn btn-primary" type="submit">${q.recommendations?'Обновить рекомендации':'Дополнить карточку и подобрать специалистов'} ${icon('arrow')}</button></div></form></section>`;
 }
 function answerCardPanel(task){
  const p=task.qualification?.cardSuggestions;if(!p)return '';
  if(p.reviewed)return `<section class="panel brief-panel"><h3>Карточка по вашим ответам</h3><p>${p.acceptedFields?.length?'Дополнения подтверждены и добавлены в поля. Рейтинг пересчитан.':p.updates.length?'Вы оставили карточку без изменений.':'Новых конкретных сведений для полей не найдено. Их можно дополнить вручную.'}</p></section>`;
  return `<section class="panel brief-panel" id="answer-card"><p class="eyebrow">КАРТОЧКА ИЗ ОТВЕТОВ · ${modeLabel(p.mode)}</p><h2>Проверьте дополненную карточку</h2><p>Агент перенёс сведения из ваших ответов в подходящие поля. Старый текст сохранён. Отметьте дополнения, которые хотите применить и подтвердить.</p>${notice(p.warning)}<form id="answer-card-form" data-task-id="${task.id}" data-set-id="${task.qualification.id}" data-revision="${task.revision}">${p.updates.map(r=>`<article class="brief-field"><label class="brief-skill"><input type="checkbox" name="fields" value="${r.field}" checked><strong>${esc(LABELS[r.field])}</strong></label><p class="field-help">Было: ${esc(r.before||'Не заполнено')}</p><blockquote style="white-space:pre-wrap">${esc(r.value)}</blockquote><details><summary>Цитаты из ваших ответов</summary>${r.evidence.map(e=>`<blockquote>${esc(e.quote)}</blockquote>`).join('')}</details></article>`).join('')}<div class="brief-actions"><button class="btn btn-primary" type="submit">Применить и подтвердить выбранные поля</button><button class="btn btn-secondary" type="submit" value="skip">Оставить без изменений</button></div></form></section>`;
 }
 ...
 app.addEventListener('submit',event=>{
  const form=event.target;if(!['brief-create-form','brief-fields-form','qualification-form','brief-skills-form','answer-card-form'].includes(form.id))return;
  ...
  run(button,async()=>{
   ...
   }else if(form.id==='answer-card-form'){
    noPendingEdits(id);await mutate(id,'brief-apply-answers',{revision:Number(form.dataset.revision),questionSetId:form.dataset.setId,confirmed:true,fields:action==='skip'?[]:new FormData(form).getAll('fields')});render();document.querySelector('.brief-fields')?.scrollIntoView({behavior:'smooth'});toast('Карточка обновлена. Подтверждённые сведения учтены в рейтинге.');
   }else if(form.id==='qualification-form'){
    noPendingEdits(id);cache()[`qualification-${id}`]=Object.fromEntries(new FormData(form));
    await mutate(id,'qualification-answers',{revision:Number(form.dataset.revision),questionSetId:form.dataset.setId,answers:cache()[`qualification-${id}`]});delete cache()[`qualification-${id}`];render();document.querySelector('#answer-card')?.scrollIntoView({behavior:'smooth'});
   }else{
